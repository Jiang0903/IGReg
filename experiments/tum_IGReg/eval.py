...
def make_parser():
    ...
    parser.add_argument('--benchmark', default='tum', help='test benchmark')
    # parser.add_argument('--method', choices=['lgr', 'ransac', 'svd'], required=True, help='registration method')
    parser.add_argument('--method', default='lgr', help='registration method')
    parser.add_argument('--num_corr', type=int, default=None, help='number of correspondences for registration')
    parser.add_argument('--verbose', action='store_true', help='verbose mode')
    return parser

def get_ply_src(scene, src):
    root = '/data/xzy/jxq/datasets/TUM/vis'
    sce_root = osp.join(root, scene)
    file_name = osp.join(sce_root, 'cloud_bin_{}.ply'.format(src))
...
def normalize_point_cloud(point_cloud):
    ...
    diagonal_length = np.linalg.norm(np.max(translated_point_cloud, axis=0) - np.min(translated_point_cloud, axis=0))
    scaling_factor = 1.0 / diagonal_length
    normalized_point_cloud = scaling_factor * translated_point_cloud
    return normalized_point_cloud


def calculate_rmse(transform, est_transform, src_points):
    transform = torch.from_numpy(transform)
    est_transform = torch.from_numpy(est_transform)
    ...
    src_points = torch.from_numpy(normalize_point_cloud(src_points.numpy()))
    realigned_src_points_f = torch.from_numpy(normalize_point_cloud(realigned_src_points_f.numpy()))
    rmse = torch.linalg.norm(realigned_src_points_f - src_points, dim=1).mean()
    
    return np.float32(rmse)
...
def eval_one_epoch(args, cfg, logger):
    ...
    registration_meter.register_meter('mean_rmse')
    registration_meter.register_meter('median_rmse')
    registration_meter.register_meter('scene_rmse')
    ...
    for scene_root in scene_roots:
        ...
        registration_meter.reset_meter('scene_rmse')
        ...
        for file_name in file_names:
            ...
            message = '{}, id0: {}, id1: {}'.format(scene_abbr, ref_frame, src_frame)
            ...
            ply_src_points = get_ply_src(scene_name, src_frame)
            # rmse = calculate_rmse(transform, estimated_transform, data_dict['src_points'])
            rmse = calculate_rmse(transform, estimated_transform, ply_src_points)
            message += ', r_RMSE: {:.8f}'.format(rmse)
            # accepted = rmse < cfg.eval.rmse_threshold ** 2
            accepted = rmse < cfg.eval.rmse_threshold
            registration_meter.update('scene_recall', float(accepted))
            registration_meter.update('scene_rmse', rmse)
            if accepted:
        ...
        mean_rmse = registration_meter.mean('scene_rmse')
        median_rmse = registration_meter.median('scene_rmse')
        registration_meter.update('mean_rmse', mean_rmse)
        registration_meter.update('median_rmse', median_rmse)
        ...
        message += ', mean_RMSE: {:.8f}'.format(mean_rmse)
        message += ', median_RMSE: {:.8f}'.format(median_rmse)
...
def main():
    ...
    cfg = make_cfg()
    log_file = osp.join(cfg.log_dir, 'eval-' + str(args.test_epoch) + '-' + args.method + '-{}.log'.format(time.strftime('%Y%m%d-%H%M%S')))
