...
class RPEConditionalTransformer(nn.Module):
    ...
    def forward(self, feats0, feats1, embeddings0_self, embeddings1_self, embeddings0_cross, embeddings1_cross, masks0=None, masks1=None):
        attention_scores = []
        for i, block in enumerate(self.blocks):
            if block == 'self':
                feats0, scores0 = self.layers[i](feats0, feats0, embeddings0_self, memory_masks=masks0)
                feats1, scores1 = self.layers[i](feats1, feats1, embeddings1_self, memory_masks=masks1)
            else:
                if self.parallel:
                    new_feats0, scores0 = self.layers[i](feats0, feats1, embeddings0_cross, embeddings1_cross, memory_masks=masks1)
                    new_feats1, scores1 = self.layers[i](feats1, feats0, embeddings1_cross, embeddings0_cross, memory_masks=masks0)
                    feats0 = new_feats0
                    feats1 = new_feats1
                else:
                    feats0, scores0 = self.layers[i](feats0, feats1, embeddings0_cross, embeddings1_cross, memory_masks=masks1)
                    feats1, scores1 = self.layers[i](feats1, feats0, embeddings1_cross, embeddings0_cross, memory_masks=masks0)
            if self.return_attention_scores:
                attention_scores.append([scores0, scores1])
        if self.return_attention_scores:
            return feats0, feats1, attention_scores
