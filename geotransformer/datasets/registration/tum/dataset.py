...
def choose_point(points, limit=20000):
    indices = np.random.permutation(points.shape[0])[: limit]
...
class TUMPairDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        image_num,
        dataset_root,
        ...
        super(TUMPairDataset, self).__init__()
    ...
    def read_int(self, scene):
        ...
        if self.subset == 'train' or self.subset == 'val':
            intrinsics = np.loadtxt(osp.join(root, 'train', scene, 'camera-intrinsics.txt'))
        else:
            intrinsics = np.loadtxt(osp.join(root, 'test', scene, 'camera-intrinsics.txt'))
    ...
    def _load_point_cloud_and_image(self, file_name, datadict):
        ...
        # image_path = point_path.replace('/projects/gr/geo/data/3DMatch/data', '/Data/3DMatch_Image')
        image_path = point_path
        image_path = image_path.replace(".pth", ".png")
        if self.image_num == 1:
    ...
    def __getitem__(self, index):
        ...
        if ref_points.shape[0] + src_points.shape[0] > 80000:
            ref_points = choose_point(ref_points)
            src_points = choose_point(src_points)
        ...
        data_dict['ref_points'] = ref_points.to(torch.float32)
        data_dict['src_points'] = src_points.to(torch.float32)
